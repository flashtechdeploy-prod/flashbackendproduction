        ...
        with engine.connect() as connection:
            ...
            if employee_data:
                ...
                try:
                    ...
                    print(f'🔍 Verification: Employee ID 7 {"exists" if exists else "missing"} in employees table')
                    
                except Exception as e:
                    if "already exists" in str(e):
                        print(f'⚠️ Employee ID {emp_dict["id"]} already exists in employees table')
                    else:
                        print(f'❌ Error creating employee: {e}')
                        return False
            else:
                print('❌ Employee ID 7 not found in employees2 table')
                return False
