#!/usr/bin/env python3
"""
Smart Data Import Tool - Handles duplicates and data type conversions
"""
...
class SmartDataImporter:
    ...
    def get_existing_ids(self, table_name, id_column='id'):
        """Get existing IDs from target table to avoid duplicates"""
        try:
            with self.target_engine.connect() as connection:
                result = connection.execute(text(f"SELECT {id_column} FROM {table_name}"))
                return set(row[0] for row in result.fetchall())
        except:
            return set()
    
    def import_from_sqlite_smart(self, sqlite_path):
        """Import data from SQLite database with duplicate handling"""
        print(f"📂 Smart Importing from SQLite: {sqlite_path}")
            ...
            for table in sqlite_tables:
                if table in self.get_target_tables():
                    ...
                    if len(df) > 0:
                        ...
                        # Handle duplicates - filter out existing IDs
                        if 'id' in df.columns:
                            existing_ids = self.get_existing_ids(table)
...
def main():
    importer = SmartDataImporter()
    
    print("🚀 Smart Data Import Tool for Flash ERP")
    print("=" * 50)
    
    sqlite_path = "C:\\Users\\ahmed\\Desktop\\backend clone\\flash_erp.db"
    
    if importer.import_from_sqlite_smart(sqlite_path):
